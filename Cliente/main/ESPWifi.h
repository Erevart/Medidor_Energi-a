...
int8_t ins_usu (lista_usuarios *red, station_info *nuevo_usu){
  ...
  if ((nuevo_usuario->ip  = (ip_addr *) os_malloc (sizeof (ip_addr))) == NULL)
    return -1;
...
#ifdef _DEBUG_WIFI
      IPAddress userip((uint32_t) (*usuario_actual)->ip->addr);
      debug.print("Direccion ip cliente ");
      debug.print(0);
      debug.print(": ");
      debug.println(userip);
...
#endif
        if ((*usuario_actual)->ip != nuevo_usuario->ip)
          (*usuario_actual)->ip = nuevo_usuario->ip;
  ...
  #ifdef _DEBUG_WIFI
          IPAddress userip((uint32_t) nuevo_usuario->ip->addr);
          debug.print("Direccion ip cliente ");
          debug.print(i);
          debug.print(": ");
          debug.println(userip);
...
void actualizacion_estado_usuarios(lista_usuarios *red){
  ...
  #ifdef _DEBUG_WIFI
   debug.println("INICIO: actualizacion lista de usuarios");
  #endif
...
#ifdef _DEBUG_WIFI
    Serial.println("Seleciono usuario registrado");
#endif
     while (infored_actual != NULL) {
      // Se comprueba ssid
#ifdef _DEBUG_WIFI
      Serial.println("Seleciono usuario nuevo");
#endif
      ...
      if (!cmp_bssid(reinterpret_cast<char*>((*usuario_actual)->bssid), reinterpret_cast<char*>(infored_actual->bssid))){
        (*usuario_actual)->estado = false;
#ifdef _DEBUG_WIFI
        debug.println("Usuario con diferente mac");
        debug.print("Direccion bssid nuevo usuario: ");
        for (int i = 0; i<6; i++){
          debug.print(infored_actual->bssid[i],HEX);
          debug.print(":");
        }
        debug.println();
        debug.print("Direccion bssid usuario registrado: ");
        for (int i = 0; i<6; i++){
          debug.print((*usuario_actual)->bssid[i],HEX);
          debug.print(":");
        }
        debug.println();
        debug.println("Comparacion mac");
        for (int i = 0; i<6; i++){
          if (infored_actual->bssid[i] == (*usuario_actual)->bssid[i]){
            debug.print("Igual: ");
            debug.println(infored_actual->bssid[i]);
          }else {
            debug.print("Diferente: ");
            debug.println(infored_actual->bssid[i]);
          }
        }
      debug.println();
            ...
            debug.println("Coincide mac");
        if ((*usuario_actual)->ip != &infored_actual->ip)
            (*usuario_actual)->ip = &infored_actual->ip;
        if (!(*usuario_actual)->estado);
          if (confirmar_conexion((*usuario_actual)->ip->addr))
              (*usuario_actual)->estado = true;
          else
              (*usuario_actual)->estado = false;
  #ifdef _DEBUG_WIFI
...
#ifdef _DEBUG_WIFI
 debug.println("FIN: actualizacion lista de usuarios");
#endif
...
#ifdef _DEBUG_WIFI
   debug.println("Se borran todos los usuarios.");
#endif
...
//  for (int i = 0; i < red->numusu; i++){
      ...
      red->numusu--;
  #ifdef _DEBUG_WIFI
         debug.print("Numero de usuarios actuales: ");
...
/*
          if (timeractred == NULL)
            timeractred = (os_timer_t*)os_malloc(sizeof(os_timer_t));
          ...
          // Se activa un timer, si transcurrido un tiempo éste no ha sido desabilitado,
          // el modo se sincronización se deshabilita y desactiva el timer.
          os_timer_setfn(timeractred,timersoft, timeractred);

          // Se activa el timer de sincronización
          os_timer_arm(timeractred, TIEMPO_CONEXION, false);
*/
       }
       else if (modo_sinc != SINCRONIZACION || modo_sin != ESPERA_USUARIOS){
